...
void MOS6502Core::OPCodesSBC(uint16_t address) {
  uint8_t value = m_pMemory->Read(address);
  uint16_t result = m_AC - value - (m_SR & CARRY ? 0 : 1);
  ...
  if (m_SR & DECIMAL) {
    if ( ((m_AC & 0x0F) - (m_SR & CARRY ? 0 : 1)) < (value & 0x0F))
      result -= 6;
...
uint16_t MOS6502Core::StackPull16() {
  uint16_t temp = m_pMemory->Read(STACK_BASE + ((m_SP + 1u) & 0xFFu)) | ((uint16_t)m_pMemory->Read(STACK_BASE + ((m_SP + 2u) & 0xFFu)) << 8u);
  m_SP += 2;
  return(temp);
}
