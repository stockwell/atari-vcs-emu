...
bool PPU::Tick(std::vector<uint8_t>& framebuffer)
{
	...
	case Render:
		if (m_cycle > 0 && m_cycle <= ScanlineVisibleDots)
		{
			...
			framebuffer[(y * ScanlineVisibleDots) + x] = m_pBus->ReadPalette(paletteAddr);
		}
...
uint8_t PPU::readOAM(uint8_t address) const
{
	return m_spriteMemory[address];
}

void PPU::writeOAM(uint8_t address, uint8_t value)
{
	m_spriteMemory[address] = value;
}
...
{
	m_pBus->Write(m_dataAddress, data);
	m_dataAddress += m_dataAddrIncrement;
}
